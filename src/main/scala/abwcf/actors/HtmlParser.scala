...
object HtmlParser {
  ...
  def apply(urlNormalizer: ActorRef[UrlNormalizer.Command]): Behavior[Command] = Behaviors.receiveMessage({
    case Parse(url, responseBody) =>
      ...
      urls.foreach(urlNormalizer ! UrlNormalizer.Normalize(_)) //TODO: Maybe debounce discovered URLs to eliminate duplicates across multiple responses (e.g. via a custom mailbox for the downstream actor)?
      Behaviors.same
  })
}
