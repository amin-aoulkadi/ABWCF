...
object PageUpdater {
  ...
  def apply(pageRepository: PageRepository): Behavior[Command | PagePersistence.UpdateCommand] = Behaviors.setup[CombinedCommand](context => {
    ...
    val maxBatchSize = config.getInt("abwcf.persistence.slick.page.update.max-batch-size")
    val maxBatchDelay = config.getDuration("abwcf.persistence.slick.page.update.max-batch-delay").toScala
